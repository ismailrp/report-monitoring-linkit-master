...
class TransactionHourly extends Page implements HasTable, HasForms
{
    use InteractsWithTable, InteractsWithForms;

    protected static string $resource = TransactionHourResource::class;
    protected static ?string $navigationIcon = 'heroicon-o-calendar';
    protected static string $view = 'filament.resources.transaction-hour-resource.pages.transaction-hourly';
    protected static ?string $navigationLabel = 'Transaction Hourly';
    protected static ?string $title = 'Transaction Hourly';
    ...
    /**
     * Build the table query. Important: normalize filters first so bindings are correct.
     */
    public function table(Table $table): Table
    {
        ...
        // Normalize date values: accept Carbon, string 'YYYY-MM-DD', or null.
        $start = null;
        ...
        // service/operator must be non-empty string; otherwise ignore
        ...
        // operator logic (apply only with normalized $operator)
        if ($operator) {
            if ($isSuperAdmin) {
                $query->where('transaction_hours.operator', $operator);
            } else {
                // non-super-admin: attempt to restrict to users who only have that operator (if mapping exists),
                // otherwise fallback to simple operator filter (so user sees results).
                if ($hasIdUser || $hasUserId) {
                    ...
                    // fallback: simple operator filter so user sees results
                    $query->where('operator', $operator);
                }
            }
        } else {
            // operator not selected: default behavior
            if (! $isSuperAdmin) {
                $ops = array_keys($this->getOperatorOptionsForUser());
                if (empty($ops)) {
                    $query->whereRaw('0 = 1');
                } else {
                    $query->whereIn('transaction_hours.operator', $ops);
                }
            }
        }

        // Ensure GROUP BY is applied (ONLY_FULL_GROUP_BY compatibility)
        $groupBys = ['transaction_hours.date', 'transaction_hours.operator', 'transaction_hours.service', 'countries.convert_usd'];
        $query->groupBy($groupBys);

        // Build select: non-aggregates (in group by) + aggregates per hour
        $selects = [
        ...
        $selects = array_merge($selects, $this->getHourlyTransactionColumns());
    ...
    private function getHourlyTransactionColumns(): array
    {
        $columns = [];
        for ($i = 24; $i >= 0; $i--) {
            $columns[] = DB::raw("COALESCE(ROUND(SUM(CASE WHEN transaction_hours.hour = {$i} THEN transaction_hours.revenue * COALESCE((CASE WHEN UPPER(countries.country) = 'OMAN' THEN countries.convert_usd / 1000 ELSE countries.convert_usd END), 1) ELSE 0 END), 0), 0) AS hour_{$i}_revenue");
        }
        return $columns;
    }
    ...
    private function getHourlyTransactionTableColumns(): array
    {
        $columns = [];
        for ($i = 24; $i >= 0; $i--) {
            $columns[] = TextColumn::make("hour_{$i}_revenue")
                ->label((string)$i)
                ->sortable()
