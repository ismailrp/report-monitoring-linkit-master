...
export default function Index({ services, count_services }: Props) {
    const handleExport = () => {
        const params = new URLSearchParams(window.location.search);
        window.location.href = `/v2/services/export?${params.toString()}`;
    };
        ...
        <AppLayout>
            ...
            <div className="mb-8 border-4 border-black bg-blue-600 p-6 text-white shadow-[8px_8px_0_0_rgba(0,0,0,1)]">
                <div className="flex justify-between items-center">
                    <div>
                        <h1 className="text-4xl font-black uppercase tracking-widest">Services</h1>
                    ...
                    <div className="flex items-center gap-6">
                        <div className="text-right">
                            <p className="text-sm font-bold text-blue-100 uppercase">Total Services</p>
                            <p className="text-4xl font-black">{count_services}</p>
                        </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-yellow-400 border-b-4 border-black text-xs uppercase tracking-wider text-black font-black">
                                <th className="px-6 py-4 border-r-2 border-black">#</th>
                                <th className="px-6 py-4 border-r-2 border-black">ID SMSGW</th>
                                <th className="px-6 py-4 border-r-2 border-black">Service Name</th>
                                <th className="px-6 py-4 border-r-2 border-black">SDC</th>
                            ...
                            {services.data.map((service) => (
                                ...
                                >
                                    ...
                                    <td className="px-6 py-4 text-right text-sm">
                                        <Link
                                            href={route('v2.services.show', service.id)}
                                            className="inline-block px-3 py-1 bg-blue-100 text-blue-800 font-bold border-2 border-black hover:bg-blue-200 shadow-[2px_2px_0_0_rgba(0,0,0,1)] active:shadow-none active:translate-x-[1px] active:translate-y-[1px] text-xs uppercase"
                                        >
                                            View
                                        </Link>
                                    </td>
