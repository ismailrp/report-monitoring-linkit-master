...
export default function Index({ data, countries, operators, services }: Props) {
    const handleExport = () => {
        ...
        window.location.href = `/v2/mo-hours/export?${params.toString()}`;
    };

    return (
        <AppLayout>
            <Head title="MO Hourly" />

            <div className="mb-8 border-4 border-black bg-emerald-600 p-6 text-white shadow-[8px_8px_0_0_rgba(0,0,0,1)]">
                <div className="flex justify-between items-center">
                    <div>
                        <h1 className="text-4xl font-black uppercase tracking-widest">MO Hourly</h1>
                        <p className="mt-2 text-emerald-100 font-bold font-mono">
                            // HOURLY MESSAGE ORIGINATED (MO)
                        </p>
                    </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-yellow-400 border-b-4 border-black text-xs uppercase tracking-wider text-black font-black">
                                ...
                                <th className="px-6 py-4 border-r-2 border-black text-right">Total Reg</th>
                                <th className="px-6 py-4 text-right">Total Unreg</th>
                            </tr>
                        </thead>
                            ...
                            {data.data.map((item) => (
                                ...
                                >
                                    ...
                                    <td className="px-6 py-4 border-r-2 border-black text-sm text-black dark:text-white text-right font-mono">
                                        {item.total_reg?.toLocaleString() || '0'}
                                    </td>
                                    <td className="px-6 py-4 text-sm text-black dark:text-white text-right font-mono">
                                        {item.total_unreg?.toLocaleString() || '0'}
                                    </td>
                                </tr>
                            ))}
                            {data.data.length === 0 && (
                                <tr>
                                    <td colSpan={6} className="px-6 py-12 text-center text-gray-500 dark:text-gray-400 font-bold italic">
