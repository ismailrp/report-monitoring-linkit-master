...
export default function Index({ data, countries, operators, services }: Props) {
    const handleExport = () => {
        ...
        window.location.href = `/v2/sr-hours/export?${params.toString()}`;
    };
    ...
    return (
        <AppLayout>
            ...
            <div className="mb-8 border-4 border-black bg-orange-600 p-6 text-white shadow-[8px_8px_0_0_rgba(0,0,0,1)]">
                <div className="flex justify-between items-center">
                    <div>
                        <h1 className="text-4xl font-black uppercase tracking-widest">SR Hourly</h1>
                        <p className="mt-2 text-orange-100 font-bold font-mono">
                            // SUCCESS RATE TRAFFIC HOURLY
                        </p>
                    </div>
                    <div className="flex items-center gap-6">
                        <div className="text-right">
                            <p className="text-sm font-bold text-orange-100 uppercase">Total Records</p>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-yellow-400 border-b-4 border-black text-xs uppercase tracking-wider text-black font-black">
                                ...
                                <th className="px-6 py-4 border-r-2 border-black text-right">Clicks</th>
                                <th className="px-6 py-4 border-r-2 border-black text-right">MO</th>
                        ...
                        <tbody className="divide-y-2 divide-black">
                            {data.data.map((item) => (
                                ...
                                >
                                    ...
                                    <td className="px-6 py-4 border-r-2 border-black text-sm text-black dark:text-white text-right font-mono">
                                        {item.total_click?.toLocaleString() || '0'}
                                    </td>
                                    <td className="px-6 py-4 border-r-2 border-black text-sm text-black dark:text-white text-right font-mono">
                                        {item.total_mo?.toLocaleString() || '0'}
                                    </td>
                                    <td className="px-6 py-4 text-sm text-right font-mono font-bold">
                                        <span className={`inline-block px-2 py-1 border-2 border-black text-xs shadow-[2px_2px_0_0_rgba(0,0,0,1)] ${Number(item.sr) >= 1
                            ...
                            {data.data.length === 0 && (
                                <tr>
                                    <td colSpan={8} className="px-6 py-12 text-center text-gray-500 dark:text-gray-400 font-bold italic">
